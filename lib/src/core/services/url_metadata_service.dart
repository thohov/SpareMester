...
class UrlMetadataService {
  ...
  static Future<String?> extractImageFromUrl(String url) async {
    try {
      ...
      final ogImagePattern1 = '<meta[^>]*property=["\']og:image["\'][^>]*content=["\']([^"\']+)["\']';
      var match = RegExp(ogImagePattern1, caseSensitive: false).firstMatch(html);
      if (match != null && match.groupCount >= 1) {
        var imageUrl = match.group(1);
        if (imageUrl != null) {
          imageUrl = _optimizeImageUrl(imageUrl);
          print('✅ Fant bilde med pattern 1 (og:image property først): $imageUrl');
          return imageUrl;
        }
      }

      // Alternative format: content first, then property
      final ogImagePattern2 = '<meta[^>]*content=["\']([^"\']+)["\'][^>]*property=["\']og:image["\']';
      match = RegExp(ogImagePattern2, caseSensitive: false).firstMatch(html);
      if (match != null && match.groupCount >= 1) {
        var imageUrl = match.group(1);
        if (imageUrl != null) {
          imageUrl = _optimizeImageUrl(imageUrl);
          print('✅ Fant bilde med pattern 2 (og:image content først): $imageUrl');
          return imageUrl;
        }
      }

      // Try Twitter card image as fallback
      final twitterImagePattern1 = '<meta[^>]*name=["\']twitter:image["\'][^>]*content=["\']([^"\']+)["\']';
      match = RegExp(twitterImagePattern1, caseSensitive: false).firstMatch(html);
      if (match != null && match.groupCount >= 1) {
        var imageUrl = match.group(1);
        if (imageUrl != null) {
          imageUrl = _optimizeImageUrl(imageUrl);
          print('✅ Fant bilde med pattern 3 (twitter:image name først): $imageUrl');
          return imageUrl;
        }
      }

      // Alternative format for Twitter
      final twitterImagePattern2 = '<meta[^>]*content=["\']([^"\']+)["\'][^>]*name=["\']twitter:image["\']';
      match = RegExp(twitterImagePattern2, caseSensitive: false).firstMatch(html);
      if (match != null && match.groupCount >= 1) {
        var imageUrl = match.group(1);
        if (imageUrl != null) {
          imageUrl = _optimizeImageUrl(imageUrl);
          print('✅ Fant bilde med pattern 4 (twitter:image content først): $imageUrl');
          return imageUrl;
        }
      }
  ...
  static String _optimizeImageUrl(String imageUrl) {
    ...
    optimized = optimized
        .replaceAll(RegExp(r'[?&]size=large', caseSensitive: false), '?size=medium')
        .replaceAll(RegExp(r'[?&]quality=high', caseSensitive: false), '?quality=medium');
